...
namespace Google.Apis.ManagedKafka.v1.Data
{
    ...
    /// <summary>
    /// Task Retry Policy is implemented on a best-effort basis. The default policy retries tasks with a minimum_backoff
    /// of 60 seconds, and a maximum_backoff of 12 hours. You can disable the policy by setting the task_retry_disabled
    /// field to true. Retry delay will be exponential based on provided minimum and maximum backoffs.
    /// https://en.wikipedia.org/wiki/Exponential_backoff. Note that the delay between consecutive task restarts may not
    /// always precisely match the configured settings. This can happen when the ConnectCluster is in rebalancing state
    /// or if the ConnectCluster is unresponsive etc. The default values for minimum and maximum backoffs are 60 seconds
    ...
    public class TaskRetryPolicy : Google.Apis.Requests.IDirectResponseSchema
    {
        ...
        /// <summary>Optional. If true, task retry is disabled.</summary>
        [Newtonsoft.Json.JsonPropertyAttribute("taskRetryDisabled")]
        public virtual System.Nullable<bool> TaskRetryDisabled { get; set; }
