                ...
                public class ServicesResource
                {
                    ...
                    public class EndpointsResource
                    {
                        ...
                        public class ListRequest : ServiceDirectoryBaseServiceRequest<Google.Apis.ServiceDirectory.v1.Data.ListEndpointsResponse>
                        {
                            ...
                            /// be `name`, `address`, `port`, or `annotations.` for map field * `` can be `&amp;lt;`,
                            /// `&amp;gt;`, `&amp;lt;=`, `&amp;gt;=`, `!=`, `=`, `:`. Of which `:` means `HAS`, and is
                            /// roughly the same as `=` * `` must be the same data type as field * `` can be `AND`,
                            /// `OR`, `NOT` Examples of valid filters: * `annotations.owner` returns endpoints that have
                            /// a annotation with the key `owner`, this is the same as `annotations:owner` *
                            ...
                            /// "endpoint-e" is returned but "endpoint-a" is not * `annotations.owner!=sd AND
                            /// annotations.foo=bar` returns endpoints that have `owner` in annotation key but value is
                            /// not `sd` AND have key/value `foo=bar` * `doesnotexist.foo=bar` returns an empty list.
                    ...
                    public class ListRequest : ServiceDirectoryBaseServiceRequest<Google.Apis.ServiceDirectory.v1.Data.ListServicesResponse>
                    {
                        ...
                        /// `name` or `annotations.` for map field * `` can be `&amp;lt;`, `&amp;gt;`, `&amp;lt;=`,
...
namespace Google.Apis.ServiceDirectory.v1.Data
{
    ...
    public class Endpoint : Google.Apis.Requests.IDirectResponseSchema
    {
        ...
        /// Annotations that fails to meet these requirements are rejected. * The `(*.)google.com/` and
        /// `(*.)googleapis.com/` prefixes are reserved for system annotations managed by Service Directory. If the user
        /// tries to write to these keyspaces, those entries are silently ignored by the system Note: This field is
        /// equivalent to the `metadata` field in the v1beta1 API. They have the same syntax and read/write to the same
    ...
    public class ResolveServiceRequest : Google.Apis.Requests.IDirectResponseSchema
    {
        ...
        /// be the same data type as field * `` can be `AND`, `OR`, `NOT` Examples of valid filters: *
        /// `annotations.owner` returns endpoints that have a annotation with the key `owner`, this is the same as
        /// `annotations:owner` * `annotations.protocol=gRPC` returns endpoints that have key/value `protocol=gRPC` *
        /// `address=192.108.1.105` returns endpoints that have this address * `port&amp;gt;8080` returns endpoints that
        /// have port number larger than 8080 *
        /// `name&amp;gt;projects/my-project/locations/us-east1/namespaces/my-namespace/services/my-service/endpoints/endpoint-c`
        /// returns endpoints that have name that is alphabetically later than the string, so "endpoint-e" is returned
        /// but "endpoint-a" is not * `annotations.owner!=sd AND annotations.foo=bar` returns endpoints that have
        /// `owner` in annotation key but value is not `sd` AND have key/value `foo=bar` * `doesnotexist.foo=bar`
        /// returns an empty list. Note that endpoint doesn't have a field called "doesnotexist". Since the filter does
        /// not match any endpoint, it returns no results For more information about filtering, see [API
    ...
    {
        /// <summary>
        /// Optional. Annotations for the service. This data can be consumed by service clients. Restrictions: * The
        /// entire annotations dictionary may contain up to 2000 characters, spread accoss all key-value pairs.
        /// Annotations that go beyond this limit are rejected * Valid annotation keys have two segments: an optional
        /// prefix and name, separated by a slash (/). The name segment is required and must be 63 characters or less,
        /// beginning and ending with an alphanumeric character ([a-z0-9A-Z]) with dashes (-), underscores (_), dots
        /// (.), and alphanumerics between. The prefix is optional. If specified, the prefix must be a DNS subdomain: a
