...
namespace Google.Apis.Safebrowsing.v5
{
    ...
    public class HashesResource
    {
        ...
        /// <summary>
        /// Search for full hashes matching the specified prefixes. This is a custom method as described by guidance at
        ...
        public class SearchRequest : SafebrowsingBaseServiceRequest<Google.Apis.Safebrowsing.v5.Data.GoogleSecuritySafebrowsingV5SearchHashesResponse>
        {
            ...
            /// <summary>Required. The hash prefixes to be looked up.</summary>
...
namespace Google.Apis.Safebrowsing.v5.Data
{
    ...
    /// https://cloud.google.com/apis/design/versioning), so clients MUST be prepared to receive FullHashDetail messages
    /// containing ThreatType enum values or ThreatAttribute enum values that are considered invalid by the client.
    /// Therefore, it is the client's responsibility to check for the validity of all ThreatType and ThreatAttribute
    /// enum values; if any value is considered invalid, the client MUST disregard the entire FullHashDetail message.
    /// </summary>
    ...
    /// <summary>
    /// The response returned after searching threat hashes. Note that if nothing is found, the server will return an OK
    /// status (HTTP status code 200) with the `full_hashes` field empty, rather than returning a NOT_FOUND status (HTTP
    /// status code 404).
    /// </summary>
    public class GoogleSecuritySafebrowsingV5SearchHashesResponse : Google.Apis.Requests.IDirectResponseSchema
    {
        /// <summary>
        /// The client-side cache duration. The client shall add this duration to the current time to determine the
        ...
        /// for a particular hash prefix, this fact should also be cached by the client. Important: the client must not
