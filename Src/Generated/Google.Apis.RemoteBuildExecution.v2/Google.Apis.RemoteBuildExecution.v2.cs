...
namespace Google.Apis.RemoteBuildExecution.v2
{
    ...
    public class ActionResultsResource
    {
        ...
        ///
        /// Implementations SHOULD ensure that any blobs referenced from the ContentAddressableStorage are available at
        /// the time of returning the ActionResult and will be for some period of time afterwards. The TTLs of the
        ...
        public class UpdateRequest : RemoteBuildExecutionBaseServiceRequest<Google.Apis.RemoteBuildExecution.v2.Data.BuildBazelRemoteExecutionV2ActionResult>
        {
            ...
            ///
            /// The particular semantics of this field is up to the server. In particular, every server will have their
            /// own supported range of priorities, and will decide how these map into retention/eviction
            /// policy.</summary>
    ...
    public class ActionsResource
    {
        ...
        /// <summary>Execute an action remotely.
        ///
        /// In order to execute an action, the client must first upload all of the inputs, the Command to run, and the
        /// Action into the ContentAddressableStorage. It then calls `Execute` with an `action_digest` referring to
        /// them. The server will run the action and eventually return the result.
        ///
        /// The input `Action`'s fields MUST meet the various canonicalization requirements specified in the
        /// documentation for their types so that it has the same digest as other logically equivalent `Action`s. The
        /// server MAY enforce the requirements and return errors if a non-canonical input is received. It MAY also
        /// proceed without verifying some or all of the requirements, such as for performance reasons. If the server
        /// does not verify the requirement, then it will treat the `Action` as distinct from another logically
        /// equivalent action if they hash differently.
        ///
        /// Returns a stream of google.longrunning.Operation messages describing the resulting execution, with eventual
        /// `response` ExecuteResponse. The `metadata` on the operation is of type ExecuteOperationMetadata.
        ///
        /// If the client remains connected after the first response is returned after the server, then updates are
        /// streamed as if the client had called WaitExecution until the execution completes or the request reaches an
        /// error. The operation can also be queried using Operations API.
        ///
        ...
        /// `MISSING` and a `subject` of `"blobs/{hash}/{size}"` indicating the digest of the missing blob.</summary>
        /// <param name="body">The body of the request.</param>
        /// <param name="instanceName">The instance of the execution system to operate against. A server may support multiple
        /// instances of the execution system (with their own workers, storage, caches, etc.). The server MAY require use of
        /// this field to select between them in an implementation-defined fashion, otherwise it can be omitted.</param>
    ...
    public class BlobsResource
    {
        ...
        /// <summary>Download many blobs at once.
        ///
        /// The server may enforce a limit of the combined total size of blobs to be downloaded using this API. This
        /// limit may be obtained using the Capabilities API. Requests exceeding the limit should either be split into
        ...
        /// <summary>Upload many blobs at once.
        ///
        /// The server may enforce a limit of the combined total size of blobs to be uploaded using this API. This limit
        /// may be obtained using the Capabilities API. Requests exceeding the limit should either be split into smaller
        /// chunks or uploaded using the ByteStream API, as appropriate.
        ///
        ...
        /// <summary>Determine if blobs are present in the CAS.
        ///
        /// Clients can use this API before uploading blobs to determine which ones are already present in the CAS and
        /// do not need to be uploaded again.
        ///
        /// There are no method-specific errors.</summary>
        /// <param name="body">The body of the request.</param>
        /// <param name="instanceName">The instance of the execution system to operate against. A server may support multiple
        /// instances of the execution system (with their own workers, storage, caches, etc.). The server MAY require use of
        /// this field to select between them in an implementation-defined fashion, otherwise it can be omitted.</param>
        ...
        /// <summary>Fetch the entire directory tree rooted at a node.
        ///
        /// This request must be targeted at a Directory stored in the ContentAddressableStorage (CAS). The server will
        /// enumerate the `Directory` tree recursively and return every node descended from the root.
        ///
        /// The GetTreeRequest.page_token parameter can be used to skip ahead in the stream (e.g. when retrying a
        /// partially completed and aborted request), by setting it to a value taken from
        /// GetTreeResponse.next_page_token of the last successfully processed GetTreeResponse).
        ///
        /// The exact traversal order is unspecified and, unless retrieving subsequent pages from an earlier request, is
        /// not guaranteed to be stable across multiple invocations of `GetTree`.
        ///
        /// If part of the tree is missing from the CAS, the server will return the portion present and omit the rest.
        ///
        /// * `NOT_FOUND`: The requested tree root is not present in the CAS.</summary>
        /// <param name="instanceName">The instance of the execution system to operate against. A server may support multiple
        /// instances of the execution system (with their own workers, storage, caches, etc.). The server MAY require use of
        /// this field to select between them in an implementation-defined fashion, otherwise it can be omitted.</param>
        ///
...
{    
    ...
    public class BuildBazelRemoteExecutionV2ResultsCachePolicy : Google.Apis.Requests.IDirectResponseSchema
    {
        ...
        /// dependent. A priority of 0 means a *default* value, decided by the server.
        ///
        /// The particular semantics of this field is up to the server. In particular, every server will have their own
        /// supported range of priorities, and will decide how these map into retention/eviction policy.</summary>
        [Newtonsoft.Json.JsonPropertyAttribute("priority")]
        public virtual System.Nullable<int> Priority { get; set; } 
