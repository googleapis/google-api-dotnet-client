...
namespace Google.Apis.RemoteBuildExecution.v2
{
    ...
    public class ActionResultsResource
    {
        ...
        /// <summary>Retrieve a cached execution result. Implementations SHOULD ensure that any blobs referenced from
        /// the ContentAddressableStorage are available at the time of returning the ActionResult and will be for some
        /// period of time afterwards. The TTLs of the referenced blobs SHOULD be increased if necessary and applicable.
        ...
        ///
        /// <param name="hash">The hash. In the case of SHA-256, it will always be a lowercase hex string exactly 64 characters
        /// long.</param>
        ...
        public class GetRequest : RemoteBuildExecutionBaseServiceRequest<Google.Apis.RemoteBuildExecution.v2.Data.BuildBazelRemoteExecutionV2ActionResult>
        {
            ...
            /// <summary>The instance of the execution system to operate against. A server may support multiple
            /// instances of the execution system (with their own workers, storage, caches, etc.). The server MAY
            /// require use of this field to select between them in an implementation-defined fashion, otherwise it can
            /// be omitted.</summary>
            ...
            /// <summary>The hash. In the case of SHA-256, it will always be a lowercase hex string exactly 64
            ...
            /// <summary>A hint to the server to inline the contents of the listed output files. Each path needs to
        ...
        /// <summary>Upload a new execution result. In order to allow the server to perform access control based on the
        /// type of action, and to assist with client debugging, the client MUST first upload the Action that produced
        /// the result, along with its Command, into the `ContentAddressableStorage`. Errors: * `INVALID_ARGUMENT`: One
        ...
        /// add the entry to the cache.</summary>
        ...
        public class UpdateRequest : RemoteBuildExecutionBaseServiceRequest<Google.Apis.RemoteBuildExecution.v2.Data.BuildBazelRemoteExecutionV2ActionResult>
        {
            ...
            /// <summary>The priority (relative importance) of this content in the overall cache. Generally, a lower
            /// value means a longer retention time or other advantage, but the interpretation of a given value is
            /// server-dependent. A priority of 0 means a *default* value, decided by the server. The particular
            /// semantics of this field is up to the server. In particular, every server will have their own supported
            /// range of priorities, and will decide how these map into retention/eviction policy.</summary>
    ...
    public class ActionsResource
    {
        ...
        /// <summary>Execute an action remotely. In order to execute an action, the client must first upload all of the
        /// inputs, the Command to run, and the Action into the ContentAddressableStorage. It then calls `Execute` with
        /// an `action_digest` referring to them. The server will run the action and eventually return the result. The
        ...
        /// `"blobs/{hash}/{size}"` indicating the digest of the missing blob.</summary>
        /// <param name="body">The body of the request.</param>
        /// <param name="instanceName">The instance of the execution system to operate against. A server may support multiple
        /// instances of the execution system (with their own workers, storage, caches, etc.). The server MAY require use of
        /// this field to select between them in an implementation-defined fashion, otherwise it can be omitted.</param>
    ...
    public class BlobsResource
    {
        ...
        /// <summary>Download many blobs at once. The server may enforce a limit of the combined total size of blobs to
        /// be downloaded using this API. This limit may be obtained using the Capabilities API. Requests exceeding the
        ...
        /// <summary>Upload many blobs at once. The server may enforce a limit of the combined total size of blobs to be
        /// uploaded using this API. This limit may be obtained using the Capabilities API. Requests exceeding the limit
        /// should either be split into smaller chunks or uploaded using the ByteStream API, as appropriate. This
        /// request is equivalent to calling a Bytestream `Write` request on each individual blob, in parallel. The
        /// requests may succeed or fail independently. Errors: * `INVALID_ARGUMENT`: The client attempted to upload
        /// more than the server supported limit. Individual requests may return the following errors, additionally: *
        ...
        /// <summary>Determine if blobs are present in the CAS. Clients can use this API before uploading blobs to
        /// determine which ones are already present in the CAS and do not need to be uploaded again. Servers SHOULD
        /// increase the TTLs of the referenced blobs if necessary and applicable. There are no method-specific
        /// errors.</summary>
        /// <param name="body">The body of the request.</param>
        /// <param name="instanceName">The instance of the execution system to operate against. A server may support multiple
        /// instances of the execution system (with their own workers, storage, caches, etc.). The server MAY require use of
        ...
        /// <summary>Fetch the entire directory tree rooted at a node. This request must be targeted at a Directory
        /// stored in the ContentAddressableStorage (CAS). The server will enumerate the `Directory` tree recursively
        /// and return every node descended from the root. The GetTreeRequest.page_token parameter can be used to skip
        /// ahead in the stream (e.g. when retrying a partially completed and aborted request), by setting it to a value
        /// taken from GetTreeResponse.next_page_token of the last successfully processed GetTreeResponse). The exact
        /// traversal order is unspecified and, unless retrieving subsequent pages from an earlier request, is not
        /// guaranteed to be stable across multiple invocations of `GetTree`. If part of the tree is missing from the
        /// CAS, the server will return the portion present and omit the rest. Errors: * `NOT_FOUND`: The requested tree
        /// root is not present in the CAS.</summary>
        /// <param name="instanceName">The instance of the execution system to operate against. A server may support multiple
        /// instances of the execution system (with their own workers, storage, caches, etc.). The server MAY require use of
        /// this field to select between them in an implementation-defined fashion, otherwise it can be omitted.</param>
        ///
    ...
    public class OperationsResource
    {
        ...
        /// <summary>Wait for an execution operation to complete. When the client initially makes the request, the
        /// server immediately responds with the current status of the execution. The server will leave the request
        /// stream open until the operation completes, and then respond with the completed operation. The server MAY
        /// choose to stream additional updates as execution progresses, such as to provide an update as to the state of
    ...
    public class V2Resource
    {
        ...
        /// <summary>GetCapabilities returns the server capabilities configuration of the remote endpoint. Only the
        /// capabilities of the services supported by the endpoint will be returned: * Execution + CAS + Action Cache
        /// endpoints should return both CacheCapabilities and ExecutionCapabilities. * Execution only endpoints should
        /// return ExecutionCapabilities. * CAS + Action Cache only endpoints should return CacheCapabilities.</summary>
        /// <param name="instanceName">The instance of the execution system to operate against. A server may support multiple
        /// instances of the execution system (with their own workers, storage, caches, etc.). The server MAY require use of
        /// this field to select between them in an implementation-defined fashion, otherwise it can be omitted.</param>
...
{    
    ...
    public class BuildBazelRemoteExecutionV2CacheCapabilities : Google.Apis.Requests.IDirectResponseSchema
    {
        ...
        /// <summary>All the digest functions supported by the remote cache. Remote cache may support multiple digest
        ...
        /// <summary>Maximum total size of blobs to be uploaded/downloaded using batch methods. A value of 0 means no
        /// limit is set, although in practice there will always be a message size limitation of the protocol in use,
        /// e.g. GRPC.</summary>
    ...
    public class BuildBazelRemoteExecutionV2ExecuteRequest : Google.Apis.Requests.IDirectResponseSchema
    {
        ...
        /// <summary>An optional policy for execution of the action. The server will have a default policy if this is
        ...
        /// <summary>An optional policy for the results of this execution in the remote cache. The server will have a
        /// default policy if this is not provided. This may be applied to both the ActionResult and the associated
        /// blobs.</summary>
        ...
        /// <summary>If true, the action will be executed even if its result is already present in the ActionCache. The
        /// execution is still allowed to be merged with other in-flight executions of the same action, however -
        /// semantically, the service MUST only guarantee that the results of an execution with this field set were not
        /// visible before the corresponding execution request was sent. Note that actions from execution requests
        /// setting this field set are still eligible to be entered into the action cache upon completion, and services
        /// SHOULD overwrite any existing entries that may exist. This allows skip_cache_lookup requests to be used as a
        /// mechanism for replacing action cache entries that reference outputs no longer available or that are poisoned
        /// in any way. If false, the result may be served from the action cache.</summary>
    ...
    {
        /// <summary>The priority (relative importance) of this action. Generally, a lower value means that the action
        /// should be run sooner than actions having a greater priority value, but the interpretation of a given value
        /// is server- dependent. A priority of 0 means the *default* priority. Priorities may be positive or negative,
        /// and such actions should run later or sooner than actions having the default priority, respectively. The
        /// particular semantics of this field is up to the server. In particular, every server will have their own
        /// supported range of priorities, and will decide how these map into scheduling policy.</summary>
    ...
    public class BuildBazelRemoteExecutionV2GetTreeResponse : Google.Apis.Requests.IDirectResponseSchema
    {
        ...
        /// <summary>If present, signifies that there are more results which the client can retrieve by passing this as
        /// the page_token in a subsequent request. If empty, signifies that this is the last page of results.</summary>
    ...
    /// <summary>Allowed values for priority in ResultsCachePolicy Used for querying both cache and execution valid
    ...
    /// <summary>A `ResultsCachePolicy` is used for fine-grained control over how action outputs are stored in the CAS
    /// and Action Cache.</summary>
    public class BuildBazelRemoteExecutionV2ResultsCachePolicy : Google.Apis.Requests.IDirectResponseSchema
    {
        /// <summary>The priority (relative importance) of this content in the overall cache. Generally, a lower value
        /// means a longer retention time or other advantage, but the interpretation of a given value is server-
        /// dependent. A priority of 0 means a *default* value, decided by the server. The particular semantics of this
        /// field is up to the server. In particular, every server will have their own supported range of priorities,
        /// and will decide how these map into retention/eviction policy.</summary>
        [Newtonsoft.Json.JsonPropertyAttribute("priority")]
        public virtual System.Nullable<int> Priority { get; set; } 
