...
namespace Google.Apis.Drive.v2
{
    ...
    public class FilesResource
    {
        ...
        /// <summary>Generates a CSE token which can be used to create or update CSE files.</summary>
        public class GenerateCseTokenRequest : DriveBaseServiceRequest<Google.Apis.Drive.v2.Data.GenerateCseTokenResponse>
        {
        ...
        /// [`files.export`](https://developers.google.com/workspace/drive/api/reference/rest/v2/files/export) instead.
        /// For more information, see [Download &amp;amp; export
        ...
        /// `files.insert` must specify the MIME type `application/vnd.google-apps.shortcut`. Apps should specify a file
        /// extension in the `title` property when inserting files with the API. For example, an operation to insert a
        /// JPEG file should specify something like `"title": "cat.jpg"` in the metadata. Subsequent `GET` requests
        /// include the read-only `fileExtension` property populated with the extension originally specified in the
        /// `title` property. When a Google Drive user requests to download a file, or when the file is downloaded
        /// through the sync client, Drive builds a full filename (with extension) based on the title. In cases where
        /// the extension is missing, Drive attempts to determine the extension based on the file's MIME type.
        /// </summary>
        /// <param name="body">The body of the request.</param>
