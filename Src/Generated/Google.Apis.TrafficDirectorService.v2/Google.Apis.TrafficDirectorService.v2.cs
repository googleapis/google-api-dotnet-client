...
namespace Google.Apis.TrafficDirectorService.v2
{
    ...
    public class TrafficDirectorServiceService : Google.Apis.Services.BaseClientService
    {
        ...
        public TrafficDirectorServiceService() :
            this(new Google.Apis.Services.BaseClientService.Initializer()) {}

        /// <summary>Constructs a new service.</summary>
        /// <param name="initializer">The service initializer.</param>
        public TrafficDirectorServiceService(Google.Apis.Services.BaseClientService.Initializer initializer)
            : base(initializer)
        {
    ...
    public abstract class TrafficDirectorServiceBaseServiceRequest<TResponse> : Google.Apis.Requests.ClientServiceRequest<TResponse>
    {
        ...
        protected TrafficDirectorServiceBaseServiceRequest(Google.Apis.Services.IClientService service)
            : base(service)
        {
        }
    ...
    public class DiscoveryResource
    {
        ...
        public class ClientStatusRequest : TrafficDirectorServiceBaseServiceRequest<Google.Apis.TrafficDirectorService.v2.Data.ClientStatusResponse>
        {
            ...
            public ClientStatusRequest(Google.Apis.Services.IClientService service, Google.Apis.TrafficDirectorService.v2.Data.ClientStatusRequest body)
                : base(service)
            {
                Body = body;
                InitParameters();
...
{    
    ...
    /// <summary>Addresses specify either a logical or physical address and port, which are used to tell Envoy where to
    /// bind/listen, connect to upstream and find management servers.</summary>
    ...
    /// <summary>BuildVersion combines SemVer version of extension with free-form build information (i.e. 'alpha',
    /// 'private-build') as a set of strings.</summary>
    ...
    /// <summary>Envoy's cluster manager fills this message with all currently known clusters. Cluster configuration
    /// information can be used to recreate an Envoy configuration by populating all clusters as static clusters or by
    /// returning them in a CDS response.</summary>
    ...
    /// <summary>Specifies the double start and end of the range using half-open interval semantics [start,
    /// end).</summary>
    ...
    {
        /// <summary>Category of the extension. Extension category names use reverse DNS notation. For instance
    ...
    /// <summary>Google's `RE2 `_ regex engine. The regex string must adhere to the documented `syntax `_. The engine is
    /// designed to complete execution in linear time as well as limit the amount of memory used. Envoy supports program
    /// size checking via runtime. The runtime keys `re2.max_program_size.error_level` and
    ...
    /// is incremented each time the program size exceeds the warn level threshold.</summary>
    public class GoogleRE2 : Google.Apis.Requests.IDirectResponseSchema
    {
        /// <summary>This field controls the RE2 "program size" which is a rough estimate of how complex a compiled
        /// regex is to evaluate. A regex that has a program size greater than the configured value will fail to
        /// compile. In this case, the configured max program size can be increased or the regex can be simplified. If
    ...
    /// <summary>Envoy's listener manager fills this message with all currently known listeners. Listener configuration
    /// information can be used to recreate an Envoy configuration by populating all listeners as static listeners or by
    /// returning them in a LDS response.</summary>
    ...
    /// <summary>Identifies a specific Envoy instance. The node identifier is presented to the management server, which
    /// may use this identifier to distinguish per Envoy configuration for serving. [#next-free-field: 12]</summary>
    public class Node : Google.Apis.Requests.IDirectResponseSchema
    {
        /// <summary>This is motivated by informing a management server during canary which version of Envoy is being
    ...
    /// <summary>Envoy's RDS implementation fills this message with all currently loaded routes, as described by their
    ...
    /// returning them in RDS responses.</summary>
    ...
    /// <summary>Envoy's scoped RDS implementation fills this message with all currently loaded route configuration
    /// scopes (defined via ScopedRouteConfigurationsSet protos). This message lists both the scopes defined inline with
    /// the higher order object (i.e., the HttpConnectionManager) and the dynamically obtained scopes via the SRDS
    /// API.</summary>
    ...
    /// <summary>Envoy uses SemVer (https://semver.org/). Major/minor versions indicate expected behaviors and APIs, the
    /// patch version field is used only for security fixes and can be generally ignored.</summary>
    ...
    {
        /// <summary>The address for this socket. :ref:`Listeners ` will bind to the address. An empty address is not
        /// allowed. Specify ``0.0.0.0`` or ``::`` to bind to any address. [#comment:TODO(zuercher) reinstate when
        /// implemented: It is possible to distinguish a Listener address via the prefix/suffix matching in
        /// :ref:`FilterChainMatch `.] When used within an upstream :ref:`BindConfig `, the address controls the source
        /// address of outbound connections. For :ref:`clusters `, the cluster type determines whether the address must
    ...
    /// <summary>StructMatcher provides a general interface to check if a given value is matched in
    /// google.protobuf.Struct. It uses `path` to retrieve the value from the struct and then check if it's matched to
    /// the specified value. For example, for the following Struct: .. code-block:: yaml fields: a: struct_value:
    /// fields: b: struct_value: fields: c: string_value: pro t: list_value: values: - string_value: m - string_value: n
    /// The following MetadataMatcher is matched as the path [a, b, c] will retrieve a string value "pro" from the
    /// Metadata which is matched to the specified prefix match. .. code-block:: yaml path: - key: a - key: b - key: c
    /// value: string_match: prefix: pr The following StructMatcher is matched as the code will match one of the string
    /// values in the list at the path [a, t]. .. code-block:: yaml path: - key: a - key: t value: list_match: one_of:
    /// string_match: exact: m An example use of StructMatcher is to match metadata in envoy.v*.core.Node.</summary>
    ...
    /// <summary>Specifies the way to match a ProtobufWkt::Value. Primitive values and ListValue are supported.
    /// StructValue is not supported and is always not matched. [#next-free-field: 7]</summary>
