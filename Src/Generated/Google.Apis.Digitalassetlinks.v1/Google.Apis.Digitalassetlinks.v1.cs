...
namespace Google.Apis.Digitalassetlinks.v1
{
    ...
    public class AssetlinksResource
    {
        ...
        /// target assets. The relation describes the intent of the link between the two assets as claimed by the source
        /// asset. An example for such relationships is the delegation of privileges or permissions. This command is
        /// most often used by infrastructure systems to check preconditions for an action. For example, a client may
        /// want to know if it is OK to send a web URL to a particular mobile app instead. The client can check for the
        /// relevant asset link from the website to the mobile app to decide if the operation should be allowed. A note
        /// about security: if you specify a secure asset as the source, such as an HTTPS website or an Android app, the
        /// API will ensure that any statements used to generate the response have been made in a secure way by the
        /// owner of that asset. Conversely, if the source asset is an insecure HTTP website (that is, the URL starts
        /// with `http://` instead of `https://`), the API cannot verify its statements securely, and it is not possible
        /// to ensure that the website's statements have not been altered by a third party. For more information, see
        /// the [Digital Asset Links technical design
        ...
        public class CheckRequest : DigitalassetlinksBaseServiceRequest<Google.Apis.Digitalassetlinks.v1.Data.CheckResponse>
        {
            ...
            /// <summary>Query string for the relation. We identify relations with strings of the format `/`, where ``
            /// must be one of a set of pre-defined purpose categories, and `` is a free-form lowercase alphanumeric
            /// string that describes the specific use case of the statement. Refer to [our API documentation](/digital-
            /// asset-links/v1/relation-strings) for the current list of supported relations. For a query to match an
            /// asset link, both the query's and the asset link's relation strings must match exactly. Example: A query
            /// with relation `delegate_permission/common.handle_all_urls` matches an asset link with relation
            /// `delegate_permission/common.handle_all_urls`.</summary>
            ...
            /// Only the schemes "http" and "https" are currently allowed. Port numbers are given as a decimal number,
            /// and they must be omitted if the standard port numbers are used: 80 for http and 443 for https. We call
            /// this limited URL the "site". All URLs that share the same scheme, hostname and port are considered to be
            /// a part of the site and thus belong to the web asset. Example: the asset with the site
            ...
            /// `https://www.google.com:444/` (port does not match) REQUIRED</summary>
    ...
    public class StatementsResource
    {
        ...
        /// statement string. The API guarantees that all statements with secure source assets, such as HTTPS websites
        /// or Android apps, have been made in a secure way by the owner of those assets, as described in the [Digital
        /// Asset Links technical design specification](https://github.com/google/digitalassetlinks/blob/master/well-
        /// known/details.md). Specifically, you should consider that for insecure websites (that is, where the URL
        /// starts with `http://` instead of `https://`), this guarantee cannot be made. The `List` command is most
        /// useful in cases where the API client wants to know all the ways in which two assets are related, or
        /// enumerate all the relationships from a particular source asset. Example: a feature that helps users navigate
        /// to related items. When a mobile app is running on a device, the feature would make it easy to navigate to
        /// the corresponding web site or Google+ profile.</summary>
        ...
        public class ListRequest : DigitalassetlinksBaseServiceRequest<Google.Apis.Digitalassetlinks.v1.Data.ListResponse>
        {
            ...
            /// <summary>Use only associations that match the specified relation. See the [`Statement`](#Statement)
            /// message for a detailed definition of relation strings. For a query to match a statement, one of the
            /// following must be true: * both the query's and the statement's relation strings match exactly, or * the
            /// query's relation string is empty or missing. Example: A query with relation
...
{    
    ...
    public class AndroidAppAsset : Google.Apis.Requests.IDirectResponseSchema
    {
        ...
        /// certificate, which in combination with the package name uniquely identifies an app. Some apps' signing keys
        /// are rotated, so they may be signed by different keys over time. We treat these as distinct assets, since we
        /// use (package name, cert) as the unique ID. This should not normally pose any problems as both versions of
        /// the app will make the same or similar statements. Other assets making statements about the app will have to
        /// be updated when a key is rotated, however. (Note that the syntaxes for publishing and querying for
    ...
    /// <summary>Uniquely identifies an asset. A digital asset is an identifiable and addressable online entity that
    /// typically provides some service or content. Examples of assets are websites, Android apps, Twitter feeds, and
    /// Plus Pages.</summary>
    ...
    public class CheckResponse : Google.Apis.Requests.IDirectResponseSchema
    {
        ...
        /// debug the result. The message will be in English and we are currently not planning to offer any
        /// translations. Please note that no guarantees are made about the contents or format of this string. Any
    ...
    /// target asset. Statements are always made by the source asset, either directly or by delegating to a statement
    /// list that is stored elsewhere. For more detailed definitions of statements and assets, please refer to our [API
    /// documentation landing page](/digital-asset-links/v1/getting-started).</summary>
    public class Statement : Google.Apis.Requests.IDirectResponseSchema
    {
        ...
        /// the person or entity who issued the statement). Every complete statement has a relation. We identify
        /// relations with strings of the format `/`, where `` must be one of a set of pre-defined purpose categories,
        /// and `` is a free-form lowercase alphanumeric string that describes the specific use case of the statement.
    ...
    {
        /// <summary>Web assets are identified by a URL that contains only the scheme, hostname and port parts. The
        /// format is http[s]://[:] Hostnames must be fully qualified: they must end in a single period ("`.`"). Only
        /// the schemes "http" and "https" are currently allowed. Port numbers are given as a decimal number, and they
        /// must be omitted if the standard port numbers are used: 80 for http and 443 for https. We call this limited
        /// URL the "site". All URLs that share the same scheme, hostname and port are considered to be a part of the
        /// site and thus belong to the web asset. Example: the asset with the site `https://www.google.com` contains
        /// all these URLs: * `https://www.google.com/` * `https://www.google.com:443/` * `https://www.google.com/foo` *
