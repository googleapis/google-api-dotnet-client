...
namespace Google.Apis.Storagetransfer.v1
{
    ...
    public class TransferJobsResource
    {
        ...
        /// running already. **Note:** The job's status field can be modified using this RPC (for example, to set a
        /// job's status to DELETED, DISABLED, or ENABLED).</summary>
        /// <param name="body">The body of the request.</param>
...
{    
    ...
    /// <summary>An AzureBlobStorageData resource can be a data source, but not a data sink. An AzureBlobStorageData
    /// resource represents one Azure container. The storage account determines the [Azure
    /// endpoint](https://docs.microsoft.com/en-us/azure/storage/common/storage-create-storage-account#storage-account-
    /// endpoints). In an AzureBlobStorageData resource, a blobs's name is the [Azure Blob Storage blob's key
    /// name](https://docs.microsoft.com/en-us/rest/api/storageservices/naming-and-referencing-containers--blobs--and-
    ...
    /// <summary>In a GcsData resource, an object's name is the Cloud Storage object's name and its "last modification
    /// time" refers to the object's `updated` property of Cloud Storage objects, which changes when the content or the
    /// metadata of the object is updated.</summary>
    ...
    {
        /// <summary>The time in UTC that no further transfer operations are scheduled. Combined with schedule_end_date,
        ...
        /// `end_time_of_day` is set and `schedule_end_date` is not set, then INVALID_ARGUMENT is returned.</summary>
        [Newtonsoft.Json.JsonPropertyAttribute("endTimeOfDay")]
        public virtual TimeOfDay EndTimeOfDay { get; set; }

        /// <summary>Interval between the start of each scheduled TransferOperation. If unspecified, the default value
        /// is 24 hours. This value may not be less than 1 hour.</summary>
