    ...
    public class ControllerResource
    {
        ...
        public class DebuggeesResource
        {
            ...
            public class BreakpointsResource
            {
                ...
                /// the controller removes them from the active list to avoid setting those breakpoints again.</summary>
                ...
                public class ListRequest : CloudDebuggerBaseServiceRequest<Google.Apis.CloudDebugger.v2.Data.ListActiveBreakpointsResponse>
                {
                    ...
                    /// `wait_expired` response field to `true` when the server-selected timeout has expired. If set to
                    /// `false` (deprecated), returns `google.rpc.Code.ABORTED` status when the server-selected timeout
                    /// has expired.</summary>
                ...
                /// `expressions` fields should not alter the breakpoint semantics. These may only make changes such as
            ...
            /// must call this method with exactly the same request content to get back the same stable `debuggee_id`.
            /// Agents should call this method again whenever `google.rpc.Code.NOT_FOUND` is returned from any
            /// controller method. This protocol allows the controller service to disable debuggees, recover from data
            /// loss, or change the `debuggee_id` format. Agents must handle `debuggee_id` value changing upon re-
        ...
        public class DebuggeesResource
        {
            ...
            public class BreakpointsResource
            {
                ...
                public class ListRequest : CloudDebuggerBaseServiceRequest<Google.Apis.CloudDebugger.v2.Data.ListBreakpointsResponse>
                {
                    ...
                    {
                        /// <summary>Capture stack frame and variables and update the breakpoint. The data is only
                        /// captured once. After that the breakpoint is set in a final state.</summary>
                        [Google.Apis.Util.StringValueAttribute("CAPTURE")]
                        CAPTURE,
                        /// <summary>Log each breakpoint hit. The breakpoint remains active until deleted or
                        /// expired.</summary>
                ...
                public class SetRequest : CloudDebuggerBaseServiceRequest<Google.Apis.CloudDebugger.v2.Data.SetBreakpointResponse>
                {
                    ...
                    public enum CanaryOptionEnum
                    {
                        ...
                        /// <summary>Enable the canary for this breakpoint if the canary_mode of the debuggee is not
                        /// CANARY_MODE_ALWAYS_ENABLED or CANARY_MODE_ALWAYS_DISABLED.</summary>
                        [Google.Apis.Util.StringValueAttribute("CANARY_OPTION_TRY_ENABLE")]
                        CANARYOPTIONTRYENABLE,
                        /// <summary>Disable the canary for this breakpoint if the canary_mode of the debuggee is not
                        /// CANARY_MODE_ALWAYS_ENABLED or CANARY_MODE_ALWAYS_DISABLED.</summary>
...
{    
    ...
    /// <summary>------------------------------------------------------------------------------ ## Breakpoint (the
    /// resource) Represents the breakpoint specification, status and results.</summary>
    ...
    /// int(7); int** pp =  { // Captured variable name: "pp", value: "0x00500500", type: "int**", members { value:
