...
{    
    ...
    ///
    /// The task will be delivered to the App Engine application hostname specified by its AppEngineHttpQueue and
    /// AppEngineHttpRequest. The documentation for AppEngineHttpRequest explains how the task's host URL is
    /// constructed.
    ///
    /// Using AppEngineHttpQueue requires [`appengine.applications.get`](https://cloud.google.com/appengine/docs/admin-
    /// api/access-control) Google IAM permission for the project and the following scope:
    ///
    ...
    /// the task by returning a successful HTTP response code ([`200` - `299`]), the task will be removed from the
    /// queue. If any other HTTP response code is returned or no response is received, the task will be retried
    ...
    /// considered.
    ///
    /// * To prevent traffic spikes and to smooth sudden increases in traffic, dispatches ramp up slowly when the queue
