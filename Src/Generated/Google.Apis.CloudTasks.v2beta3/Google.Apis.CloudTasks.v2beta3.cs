...
namespace Google.Apis.CloudTasks.v2beta3.Data
{
    ...
    public class Queue : Google.Apis.Requests.IDirectResponseSchema
    {
        ...
        /// from a first attempt or a retry). * retry_config controls what happens to a particular task after its first
    ...
    {
        /// <summary>
        /// Number of attempts per task, including the first attempt. (If the first attempt fails, there will be
        /// `max_attempts - 1` retries.) Must be greater than or equal to -1, which indicates unlimited attempts. Cloud
        /// Tasks stops retrying only when `max_attempts` and `max_retry_duration` are both satisfied, or when the task
        /// is successfully executed. When the task has been attempted `max_attempts` times and when the
        /// `max_retry_duration` time has passed, no further attempts are made, and the task is deleted. If
        /// `max_attempts` is set to -1 and `max_retry_duration` is set to 0, the task is retried until the [maximum
        /// task retention](https://docs.cloud.google.com/tasks/docs/quotas#limits) limit is reached. If unspecified
        /// when the queue is created, Cloud Tasks will pick the default. This field has the same meaning as
        /// [task_retry_limit in
        ...
        /// queue's RetryConfig specifies that the task should be retried. The value must be given as a string that
        /// indicates the length of time (in seconds) followed by `s` (for "seconds"). For more information on the
        ...
        /// truncated to the nearest second. If unspecified when the queue is created, Cloud Tasks will pick the
        /// default. This field has the same meaning as [max_backoff_seconds in
        /// queue.yaml/xml](https://cloud.google.com/appengine/docs/standard/python/config/queueref#retry_parameters).
        /// </summary>
        ...
        /// max_attempts times, no further attempts are made and the task is deleted. A zero (0) indicates an unlimited
        /// duration, up to the [maximum task retention](https://docs.cloud.google.com/tasks/docs/quotas#limits) limit.
        /// The value must be given as a string that indicates the length of time (in seconds) followed by `s` (for
        ...
        /// truncated to the nearest second. If unspecified when the queue is created, Cloud Tasks will pick the
        /// default. This field has the same meaning as [task_age_limit in
        /// queue.yaml/xml](https://cloud.google.com/appengine/docs/standard/python/config/queueref#retry_parameters).
        /// </summary>
        ...
        /// truncated to the nearest second. If unspecified when the queue is created, Cloud Tasks will pick the
        /// default. This field has the same meaning as [min_backoff_seconds in
        /// queue.yaml/xml](https://cloud.google.com/appengine/docs/standard/python/config/queueref#retry_parameters).
        /// </summary>
