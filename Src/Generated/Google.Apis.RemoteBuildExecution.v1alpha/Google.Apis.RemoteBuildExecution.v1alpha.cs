    ...
    public class ProjectsResource
    {
        ...
        public class InstancesResource
        {
            ...
            public class WorkerpoolsResource
            {
                ...
                /// <summary>
                /// Creates a new worker pool with a specified size and configuration. Returns a long running operation
                /// which contains a worker pool on completion. While the long running operation is in progress, any
                /// call to `GetWorkerPool` returns a worker pool in state `CREATING`.
                /// </summary>
                ...
                /// call to `GetWorkerPool` returns a worker pool in state `DELETING`.
                /// </summary>
                /// <param name="name">
                ...
                public class ListRequest : RemoteBuildExecutionBaseServiceRequest<Google.Apis.RemoteBuildExecution.v1alpha.Data.GoogleDevtoolsRemotebuildexecutionAdminV1alphaListWorkerPoolsResponse>
                {
                    ...
                    /// `&amp;lt;`. The `:` operator can be used with string fields to match substrings. For non-string
                    /// fields it is equivalent to the `=` operator. The `:*` comparison can be used to test whether a
                    /// key has been defined. You can also filter on nested fields. To filter on multiple expressions,
                    /// you can separate expression using `AND` and `OR` operators, using parentheses to specify
                    /// precedence. If neither operator is specified, `AND` is assumed. Examples: Include only pools
                    /// with more than 100 reserved workers: `(worker_count &amp;gt; 100) (worker_config.reserved =
                    /// true)` Include only pools with a certain label or machines of the e2-standard family:
                ...
                /// progress, any call to `GetWorkerPool` returns a worker pool in state `UPDATING`.
                /// </summary>
                /// <param name="body">The body of the request.</param>
                /// <param name="name">
            ...
            /// <summary>
            /// Updates the specified instance. Returns a long running operation which contains the updated instance in
