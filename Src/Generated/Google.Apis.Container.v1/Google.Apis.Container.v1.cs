...
{    
    ...
    public class NodeConfig : Google.Apis.Requests.IDirectResponseSchema
    {
        ...
        /// - "user-profile-psm1" The following keys are reserved for Windows nodes: - "serial-port-logging-enable"
        /// Values are free-form strings, and only have meaning as interpreted by the image running in the instance. The
        /// only restriction placed on them is that each value's size must be less than or equal to 32 KB. The total
        /// size of all keys and values must be less than 512 KB.</summary>
