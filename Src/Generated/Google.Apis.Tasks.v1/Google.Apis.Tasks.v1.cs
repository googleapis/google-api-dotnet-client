...
namespace Google.Apis.Tasks.v1
{
    ...
    public class TasksResource
    {
        ...
        public class MoveRequest : TasksBaseServiceRequest<Google.Apis.Tasks.v1.Data.Task>
        {
            ...
            /// The task set as parent must exist in the task list and can not be hidden. Exceptions: 1. Assigned and
            /// repeating tasks cannot be set as parent tasks (have subtasks), or be moved under a parent task (become
            /// subtasks). 2. Tasks that are both completed and hidden cannot be nested, so the parent field must be
            /// empty.
            /// </summary>
            [Google.Apis.Util.RequestParameterAttribute("parent", Google.Apis.Util.RequestParameterType.Query)]
            public virtual string Parent { get; set; }
...
namespace Google.Apis.Tasks.v1.Data
{
    ...
    public class Task : Google.Apis.Requests.IDirectResponseSchema
    {
        ...
        /// <summary>
        /// Scheduled date for the task (as an RFC 3339 timestamp). Optional. This represents the day that the task
        /// should be done, or that the task is visible on the calendar grid. It doesn't represent the deadline of the
        /// task. Only date information is recorded; the time portion of the timestamp is discarded when setting this
        /// field. It isn't possible to read or write the time that a task is scheduled for using the API.
        /// </summary>
