...
{    
    ...
    public class ResourceRequirements : Google.Apis.Requests.IDirectResponseSchema
    {
        ...
        /// values for CPU are '1', '2', and '4'. Setting 4 CPU requires at least 2Gi of memory. Cloud Run for Anthos:
        /// supported Limits describes the maximum amount of compute resources allowed. The values of the map is string
    ...
    public class RevisionTemplate : Google.Apis.Requests.IDirectResponseSchema
    {
        ...
        /// the Configuration. The following annotation keys set properties of the created revision: *
        /// `autoscaling.knative.dev/minScale` sets the minimum number of instances. *
        /// `autoscaling.knative.dev/maxScale` sets the maximum number of instances. * `run.googleapis.com/cloudsql-
        /// instances` sets Cloud SQL connections. Multiple values should be comma separated. * `run.googleapis.com/vpc-
        /// access-connector` sets a Serverless VPC Access connector. * `run.googleapis.com/vpc-access-egress` sets VPC
        /// egress. Supported values are `all` and `private-ranges-only`.</summary>
        [Newtonsoft.Json.JsonPropertyAttribute("metadata")]
        public virtual ObjectMeta Metadata { get; set; }
