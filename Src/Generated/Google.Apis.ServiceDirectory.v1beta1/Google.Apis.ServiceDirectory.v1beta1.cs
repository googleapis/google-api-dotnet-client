...
{    
    ...
    public class Endpoint : Google.Apis.Requests.IDirectResponseSchema
    {
        ...
        /// <summary>Optional. Metadata for the endpoint. This data can be consumed by service clients. Restrictions: -
        /// The entire metadata dictionary may contain up to 512 characters, spread accoss all key-value pairs. Metadata
        ...
        /// tries to write to these keyspaces, those entries will be silently ignored by the system.</summary>
    ...
    public class Service : Google.Apis.Requests.IDirectResponseSchema
    {
        ...
        /// <summary>Optional. Metadata for the service. This data can be consumed by service clients. Restrictions: -
        /// The entire metadata dictionary may contain up to 2000 characters, spread accoss all key-value pairs.
        /// Metadata that goes beyond any these limits will be rejected. - Valid metadata keys have two segments: an
        /// optional prefix and name, separated by a slash (/). The name segment is required and must be 63 characters
        /// or less, beginning and ending with an alphanumeric character ([a-z0-9A-Z]) with dashes (-), underscores (_),
        /// dots (.), and alphanumerics between. The prefix is optional. If specified, the prefix must be a DNS
        /// subdomain: a series of DNS labels separated by dots (.), not longer than 253 characters in total, followed
        /// by a slash (/). Metadata that fails to meet these requirements will be rejected. - The '(*.)google.com/' and
        /// '(*.)googleapis.com/' prefixes are reserved for system metadata managed by Service Directory. If the user
        /// tries to write to these keyspaces, those entries will be silently ignored by the system.</summary>
        [Newtonsoft.Json.JsonPropertyAttribute("metadata")]
        public virtual System.Collections.Generic.IDictionary<string, string> Metadata { get; set; }
