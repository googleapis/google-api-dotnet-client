...
namespace Google.Apis.SemanticTile.v1
{
    ...
    public class FeaturetilesResource
    {
        ...
        /// <param name="name">Required. Resource name of the tile. The tile resource name is prefixed by its collection ID
        /// `tiles/` followed by the resource ID, which encodes the tile's global x and y coordinates and zoom level as `@,,z`.
        /// For example, `tiles/@1,2,3z`.</param>
        ...
        public class GetRequest : SemanticTileBaseServiceRequest<Google.Apis.SemanticTile.v1.Data.FeatureTile>
        {
            ...
            /// <summary>Required. Resource name of the tile. The tile resource name is prefixed by its collection ID
            /// `tiles/` followed by the resource ID, which encodes the tile's global x and y coordinates and zoom level
            /// as `@,,z`. For example, `tiles/@1,2,3z`.</summary>
            [Google.Apis.Util.RequestParameterAttribute("name", Google.Apis.Util.RequestParameterType.Path)]
            public virtual string Name { get; private set; }

            /// <summary>API client name and version. For example, the SDK calling the API. The exact format is up to
            /// the client.</summary>
            ...
            /// <summary>Application version number, such as "1.2.3". The exact format is application-
            /// dependent.</summary>
            ...
            /// <summary>Operating system name and version as reported by the OS. For example, "Mac OS X 10.10.4". The
            /// exact format is platform-dependent.</summary>
            ...
            /// <summary>Flag indicating whether human-readable names should be returned for features. If this is set,
            /// the display_name field on the feature will be filled out.</summary>
            ...
            /// <summary>Flag indicating whether 3D building models should be enabled. If this is set structures will be
            /// returned as 3D modeled volumes rather than 2.5D extruded areas where possible.</summary>
            ...
            /// <summary>Flag indicating whether the returned tile will contain road features that are marked private.
            /// Private roads are indicated by the Feature.segment_info.road_info.is_private field.</summary>
            ...
            /// <summary>Flag indicating whether unclipped buildings should be returned. If this is set, building render
            /// ops will extend beyond the tile boundary. Buildings will only be returned on the tile that contains
            /// their centroid.</summary>
    ...
    public class TerraintilesResource
    {
        ...
        /// <param name="name">Required. Resource name of the tile. The tile resource name is prefixed by its collection ID
        /// `terraintiles/` followed by the resource ID, which encodes the tile's global x and y coordinates and zoom level as
        /// `@,,z`. For example, `terraintiles/@1,2,3z`.</param>
        public virtual GetRequest Get(string name)
        {
            return new GetRequest(service, name);
        }
        ...
        public class GetRequest : SemanticTileBaseServiceRequest<Google.Apis.SemanticTile.v1.Data.TerrainTile>
        {
            ...
            /// zoom level as `@,,z`. For example, `terraintiles/@1,2,3z`.</summary>
            [Google.Apis.Util.RequestParameterAttribute("name", Google.Apis.Util.RequestParameterType.Path)]
            public virtual string Name { get; private set; }

            /// <summary>The precision of terrain altitudes in centimeters. Possible values: between 1 (cm level
            /// precision) and 1,000,000 (10-kilometer level precision).</summary>
...
{    
    ...
    /// <summary>Information about the data providers that should be included in the attribution string shown by the
    /// client.</summary>
    ...
    /// <summary>Represents a relation to another feature in the tile. For example, a building might be occupied by a
    /// given POI. The related feature can be retrieved using the related feature index.</summary>
    ...
    {
        /// <summary>The difference between each successive pair of altitudes, from west to east. The first, westmost
        /// point, is just the altitude rather than a diff. The units are specified by the altitude_multiplier parameter
        /// above; the value in meters is given by altitude_multiplier * altitude_diffs[n]. The altitude row (in metres
    ...
    /// <summary>3D vertex list used for modeled volumes. Each entry represents an offset from the previous one in local
    /// tile coordinates. The first coordinate is offset from (0, 0, 0).</summary>
