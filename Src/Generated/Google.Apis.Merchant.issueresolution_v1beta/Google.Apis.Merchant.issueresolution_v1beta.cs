...
namespace Google.Apis.Merchant.issueresolution_v1beta
{
    ...
    public class IssueresolutionResource
    {
        ...
        /// <param name="body">The body of the request.</param>
        /// <param name="name">
        /// Required. The name of the product. Format: `accounts/{account}/products/{product}` The `{product}` segment
        /// is a unique identifier for the product. This identifier must be unique within a merchant account and
        /// generally follows the structure: `content_language~feed_label~offer_id`. Example: `en~US~sku123` For legacy
        /// local products, the structure is: `local~content_language~feed_label~offer_id`. Example:
        /// `local~en~US~sku123` The format of the `{product}` segment in the URL is automatically detected by the
        /// server, supporting two options: 1. **Encoded Format**: The `{product}` segment is an unpadded base64url
        ...
        /// product ID `en~US~sku/123`, the `{product}` segment must be the base64url encoding of this string, which is
        ...
        /// those containing special characters. The presence of tilde (`~`) characters in the `{product}` segment is
        /// used to differentiate between the two formats. Note: For calls to the v1beta version, the plain format is
        /// `channel~content_language~feed_label~offer_id`, for example: `accounts/123/products/online~en~US~sku123`.
        /// </param>
        ...
        public class RenderproductissuesRequest : MerchantBaseServiceRequest<Google.Apis.Merchant.issueresolution_v1beta.Data.RenderProductIssuesResponse>
        {
            ...
            /// segment is a unique identifier for the product. This identifier must be unique within a merchant account
            /// and generally follows the structure: `content_language~feed_label~offer_id`. Example: `en~US~sku123` For
            ...
            /// `content_language~feed_label~offer_id` structure. This encoding MUST be used if any part of the product
            /// identifier (like `offer_id`) contains characters such as `/`, `%`, or `~`. * Example: To represent the
            /// product ID `en~US~sku/123`, the `{product}` segment must be the base64url encoding of this string, which
            ...
            /// `%`, or `~`. We recommend using the **Encoded Format** for all product IDs to ensure correct parsing,
            /// especially those containing special characters. The presence of tilde (`~`) characters in the
            /// `{product}` segment is used to differentiate between the two formats. Note: For calls to the v1beta
            /// version, the plain format is `channel~content_language~feed_label~offer_id`, for example:
        ...
        /// user input form. Access to the `triggeraction` method is restricted to an allowlist. You can request access
        /// using [Trigger action allowlist
        /// form](https://docs.google.com/forms/d/e/1FAIpQLSfeV_sBW9MBQv9BMTV6JZ1g11PGHLdHsrefca-9h0LmpU7CUg/viewform?usp=sharing).
        /// The action can be successfully started only once all `required` inputs are provided. If any `required` input
        /// is missing, or invalid value was provided, the service will return 400 error. Validation errors will contain
        /// Ids for all problematic field together with translated, human readable error messages that can be shown to
        /// the user.
        /// </summary>
        /// <param name="body">The body of the request.</param>
        /// <param name="name">
