...
namespace Google.Apis.Merchant.issueresolution_v1beta
{
    ...
    public class IssueresolutionResource
    {
        ...
        /// <param name="body">The body of the request.</param>
        /// <param name="name">
        /// Required. The name of the product. Format: `accounts/{account}/products/{product}` The {product} segment is
        /// a unique identifier for the product. This identifier must be unique within a merchant account and generally
        /// follows the structure: `content_language~feed_label~offer_id`. Example: `en~US~sku123` For legacy local
        /// products, the structure is: `local~content_language~feed_label~offer_id`. Example: `local~en~US~sku123` The
        /// format of the {product} segment in the URL is automatically detected by the server, supporting two options:
        /// 1. **Encoded Format**: The `{product}` segment is an unpadded base64url encoded string (RFC 4648 Section 5).
        ...
        /// `~`. * Example: To represent the product ID `en~US~sku/123`, the `{product}` segment must be the base64url
        /// encoding of this string, which is `ZW5-VVMtc2t1LzEyMw`. The full resource name for the product would be
